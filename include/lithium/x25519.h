 ...
 * clamp is set. This indicates "non-contributory behavior", meaning that one
 * party might steer the key so that the other party's contribution doesn't
 * matter, or contributes only a little entropy.
 *
 ...
 *
 * Per RFC 7748, this function returns failure if the output is zero and clamp
 * is set. This usually doesn't matter for base scalarmuls.
 *
 * If clamp==0, then this function always returns 0.
 *
 * Same as x25519(out,scalar,x25519_base_point), except that other
 ...
 *
 * This is used for signing. Implementors must replace it for curves that
 * require more bytes for uniformity (Brainpool).
 */
